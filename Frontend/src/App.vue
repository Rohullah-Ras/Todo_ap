...
function enhanceTask(t) {
  const statusName = t.status?.name ?? t.statusName ?? null
...
async function loadBoard() {
  ...
  try {
    ...
    for (const t of tasks) {
      const key = t.list?.key ?? (listsById.value[t.listId]?.key ?? 'todo')
      if (!columns.value[key]) columns.value[key] = []
      columns.value[key].push(enhanceTask(t))
    }
...
async function onDragEnd(evt) {
  ...
  try {
    await axios.patch(`${TASKS_URL}/${movedTask.id}`, {
      listId: list.id,
